  ...
  const captureFromCamera = useCallback((): Promise<ImageUploadResult | null> => {
    return new Promise((resolve) => {
      try {
        const input = document.createElement('input');
        input.type = 'file';
        input.accept = 'image/*'; // Broader accept for better mobile compatibility
        ...
        input.onchange = async (e) => {
          try {
            ...
            if (file) {
              ...
              console.log('❌ No file captured from camera');
        ...
        input.onerror = (err) => {
          console.error('❌ Input error event:', err);
        ...
        console.error('❌ Unable to create camera input:', err);

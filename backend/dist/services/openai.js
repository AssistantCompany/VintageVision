 ...
 */
function safeJsonParse(content, stage) {
    try {
        return JSON.parse(content);
    }
    catch (error) {
        console.error(`❌ JSON parse error in ${stage}:`, content.substring(0, 200));
        throw new ExternalServiceError(`Failed to parse ${stage} response as JSON. The AI may have returned malformed data.`, 'OpenAI');
    }
}
// ============================================================================
// SCHEMA DEFINITIONS
...
const WorldClassAnalysisSchema = z.object({
    ...
    stylingSuggestions: z.array(z.object({
        title: z.string(),
        description: z.string(),
        roomType: z.string(),
...
// ============================================================================
async function performTriage(imageBase64) {
...
Respond in JSON:
{
  ...
  "allVisibleText": ["Array of ALL text visible in image"]
    ...
    if (!content)
        throw new ExternalServiceError('No triage response', 'OpenAI');
...
// ============================================================================
async function extractEvidence(imageBase64, triage) {
...
Every detail matters for precise identification and valuation.`,
    ...
    const content = response.choices[0]?.message?.content;
    if (!content)
        throw new ExternalServiceError('No evidence response', 'OpenAI');
...
// ============================================================================
async function generateCandidates(imageBase64, triage, evidence) {
...
Respond in JSON:
{
  "candidates": [
    ...
    const content = response.choices[0]?.message?.content;
    if (!content)
        throw new ExternalServiceError('No candidates response', 'OpenAI');
    const parsed = safeJsonParse(content, 'candidates');
...
// ============================================================================
async function generateFinalAnalysis(imageBase64, triage, evidence, candidates, askingPrice) {
...
Respond in JSON:
{
  ...
  "stylingSuggestions": [
    ...
    const content = response.choices[0]?.message?.content;
    if (!content)
        throw new ExternalServiceError('No final analysis response', 'OpenAI');
...
async function generateAuthenticationAnalysis(imageBase64, triage, evidence, analysis) {
    ...
    const isLuxuryBrand = analysis.maker?.toLowerCase().match(/rolex|patek|omega|cartier|tiffany|louis vuitton|hermes|chanel|audemars|vacheron|breguet/);
...
Respond in JSON:
{
  ...
  "overallAssessment": "2-3 sentence summary of authentication status and key concerns"
    ...
    const content = response.choices[0]?.message?.content;
    if (!content)
        throw new ExternalServiceError('No authentication response', 'OpenAI');
...
// MAIN ORCHESTRATOR
// ============================================================================
export async function analyzeAntiqueImage(imageBase64, askingPrice) {
    try {
        ...
        // Base64 overhead is ~33%, so 30MB base64 ≈ 22MB raw image
        ...
        console.log('');
        console.log('════════════════════════════════════════════════════════════════');
        console.log('🏺 VINTAGEVISION WORLD-CLASS IDENTIFICATION SYSTEM v5.0 (GPT-5.2)');
        console.log('════════════════════════════════════════════════════════════════');
        if (askingPrice) {
        ...
        const evidence = await extractEvidence(imageBase64, triage);
        // Stage 3: Candidate Generation
        const candidates = await generateCandidates(imageBase64, triage, evidence);
        if (candidates.length === 0) {
            throw new ExternalServiceError('Could not generate identification candidates', 'OpenAI');
        }
        // Stage 4: Final Analysis
        const analysis = await generateFinalAnalysis(imageBase64, triage, evidence, candidates, askingPrice);
        // Stage 5: Authentication Analysis
        const authAnalysis = await generateAuthenticationAnalysis(imageBase64, triage, evidence, analysis);
        // Merge authentication data into result
        const fullResult = {
            ...analysis,
            authenticationConfidence: authAnalysis.authenticationConfidence,
            authenticityRisk: authAnalysis.authenticityRisk,
            authenticationChecklist: authAnalysis.checklist,
            knownFakeIndicators: authAnalysis.knownFakeIndicators,
            additionalPhotosRequested: authAnalysis.photosRequested,
            expertReferralRecommended: authAnalysis.expertReferralRecommended,
            expertReferralReason: authAnalysis.expertReferralReason,
            authenticationAssessment: authAnalysis.overallAssessment,
        };
        console.log('');
        console.log('════════════════════════════════════════════════════════════════');
        console.log(`✅ IDENTIFICATION & AUTHENTICATION COMPLETE`);
        console.log(`   Item: ${fullResult.name}`);
        if (fullResult.maker)
            console.log(`   Maker: ${fullResult.maker}`);
...
// MARKETPLACE LINKS
// ============================================================================
export function generateMarketplaceLinks(itemName, era, estimatedValueMin, productCategory, brand, modelNumber, domainExpert) {
    ...
    if (domainExpert === 'furniture') {
        links.push({ marketplaceName: 'Chairish', linkUrl: `https://www.chairish.com/search?q=${searchTerm}` }, { marketplaceName: '1stDibs', linkUrl: `https://www.1stdibs.com/search/?q=${searchTerm}` });
    }
    else if (domainExpert === 'art') {
        links.push({ marketplaceName: 'Artnet', linkUrl: `https://www.artnet.com/search/?q=${searchTerm}` }, { marketplaceName: '1stDibs', linkUrl: `https://www.1stdibs.com/search/?q=${searchTerm}` });
    }
    else if (domainExpert === 'jewelry' || domainExpert === 'watches') {
        links.push({ marketplaceName: '1stDibs', linkUrl: `https://www.1stdibs.com/search/?q=${searchTerm}` }, { marketplaceName: 'Ruby Lane', linkUrl: `https://www.rubylane.com/search?q=${searchTerm}` });
    }
    else if (domainExpert === 'ceramics' || domainExpert === 'glass') {
        links.push({ marketplaceName: 'Replacements', linkUrl: `https://www.replacements.com/search?query=${searchTerm}` }, { marketplaceName: 'Ruby Lane', linkUrl: `https://www.rubylane.com/search?q=${searchTerm}` });
    }
    else if (domainExpert === 'books') {
        links.push({ marketplaceName: 'AbeBooks', linkUrl: `https://www.abebooks.com/servlet/SearchResults?kn=${searchTerm}` }, { marketplaceName: 'Biblio', linkUrl: `https://www.biblio.com/search.php?keyisbn=${searchTerm}` });
    }
    else {
        links.push({ marketplaceName: 'Ruby Lane', linkUrl: `https://www.rubylane.com/search?q=${searchTerm}` }, { marketplaceName: '1stDibs', linkUrl: `https://www.1stdibs.com/search/?q=${searchTerm}` });
    }
    return links;
}
// ============================================================================

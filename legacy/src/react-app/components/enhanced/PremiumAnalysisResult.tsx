...
import { cn, formatCurrency, shareContent, copyToClipboard, trackEvent, vibrate } from '@/react-app/lib/utils'
...
export default function PremiumAnalysisResult({ 
  ...
  const handleShare = async () => {
    ...
    const shareData = {
      ...
      text: `I discovered this ${analysis.name} ${analysis.era ? `from the ${analysis.era}` : ''}${analysis.estimated_value_min ? ` worth $${analysis.estimated_value_min.toLocaleString()}+` : ''} using VintageVision AI!`,
                      ...
                      {(analysis.estimated_value_min || analysis.estimated_value_max) && (
                        <span className="px-4 py-2 bg-emerald-100 text-emerald-800 rounded-full text-sm font-medium flex items-center gap-2">
                          ...
                          {analysis.estimated_value_min && analysis.estimated_value_max
                            ...
                            : analysis.estimated_value_min
                              ? `${formatCurrency(analysis.estimated_value_min)}+`
                              : `Up to ${formatCurrency(analysis.estimated_value_max!)}`
                          }
                        </span>
                      )}
                    </div>
